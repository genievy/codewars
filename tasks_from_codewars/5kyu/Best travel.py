...
def choose_best_sum(t, k, xs):
    dist = [i for i in map(sum, list(combinations(xs, k))) if i <= t]
    if dist:
        return max(dist)
    else:
        return None
...
xs = [50, 55, 57, 58, 60] # --> [50,55,57],[50,55,58],[50,55,60],[50,57,58],[50,57,60],[50,58,60],[55,57,58],[55,57,60],[55,58,60],[57,58,60]
