...
def valid_solution(board):
    l1, l2, l3 = [], [], []
    for n in range(0, 9):
        l1.append(sum([board[n][m] for m in range(0, 9) if board[n][m] !=0]))
        l2.append(sum([board[m][n] for m in range(0, 9) if board[n][m] !=0]))
    for d in range(0, 9, 3):
        for k in range(0, 9, 3):
            lp = []
            for m in range(0 + d, 3+ d):
                lp.append(sum([board[m][n] for n in range(0 + k, 3 + k)]))
            l3.append(sum(lp))
    return len(set(l1)) == len(set(l2)) == len(set(l3)) == 1
